...
def hab6(): #WARNING DELIVERY FAILURE, DO NOT USE, DEPRICATED
    hab(wait=1, crosstime=10, rewardtrigger=0)

def pick_random():
    num = random.randint(25, 35)
    print("random iti", num)
    return num

# replaced the rewarder with the trigger and took out the thrid state
def hab(wait, crosstime, rewardtrigger):
    ...
    while time.time() < endtime:
        ...
        while state == 2:  # state 2: trigger trigger/prime reward 
            if rew.isCrossed() and time.time() > wait+start:
                # trigRun.value = 0
                rewRun.value = 0
                deadline = time.time()+crosstime
                start = time.time()
                if rewardtrigger == 1:
                   lines[0].deliver()
                   print("rewarder delivered")
                state = 0  # moved this to after delivery
                print("state 0")
            if not rew.isCrossed(): #figure out how to end program at endtime 
                rewRun.value = 0
                lines[line].killTone()
                state = 0
                print("state 0")
...
rew = nosePoke(40, 38, 'reward')
trig = trigger(13, 15, 'trigger')
rew.flashOff()
trig.flashOff()
